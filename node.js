...
module.exports = function (RED) {
    function FunctionNode(n) {
        ...
        this.on('input', function (msg) {
            ...
            var url = 'https://api.bithumb.com';
            var endPoint = '';
            if(node.params.category){
                url += '/' + node.params.category;
                endPoint += '/' + node.params.category;
            }
            if(node.params.api){
                url += '/' + node.params.api;
                endPoint += '/' + node.params.api;
            }
            node.params.endPoint = endPoint;
            ...
            var headers = _getHttpHeaders(endPoint, node.params, node.apiKey, node.secretKey);
    ...
    RED.nodes.registerType("bithumb", FunctionNode, {
        credentials: {
            apiKey: {type:"text"},
            secretKey: {type:"text"}
        }
    ...
    function bithumbApiKey(n){
        RED.nodes.createNode(this, n);
        this.apiKey = n.apiKey;
        this.secretKey = n.secretKey;
    }

    RED.nodes.registerType("bithumbApiKey", bithumbApiKey,{
        credentials: {
            apiKey: {type:"text"},
            secretKey: {type:"text"}
        }
...
function _getHttpHeaders(endPoint, rgParams, api_key, api_secret) {
